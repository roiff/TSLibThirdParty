...
function setAirplaneMode(flag) end
...
function setVPNEnable(flag) end
...
function setWifiEnable(flag) end
...
function setReduceMotionEnable(flag) end
...
function setCellularDataEnable(flag) end
...
function setAssistiveTouchEnable(flag) end
...
function getBacklightLevel() end
...
function setBacklightLevel(light) end
...
function setVolumeLevel(vol) end

--- 
--- 手机振动，震动
-- - 函数功能：使手机振动一次
function vibrator() end

--- 
--- 播放音频
-- - 函数功能：在后台播放指定的声音文件
---@param filename string @音频文件名,支持 mp3、m4a、m4r、ogg、wav,iOS 填写""停止播放
function playAudio(filename) end
...
function setScreenResolution(size) end
...
--- 退出脚本、结束函数
-- - 函数功能：直接终止脚本
function lua_exit() end

--- 
--- 退出脚本、结束函数
-- - 函数功能：直接终止脚本
function luaExit() end

--- 
--- 重新加载运行脚本
-- - 引擎版本：iOS v2.2.1,Android v1.1.9 以上
function lua_restart() end

--- 
--- TSImgVersion 获取版本号
-- - 函数功能：获取 tsimg.so 版本号
---@return string @版本号
function image.TSImgVersion() end

--- 
--- 终止之前运行
-- - 函数功能：在用户主动终止脚本运行之前执行
-- - 引擎版本：iOS v2.2.1,Android v1.1.9 以上
function beforeUserExit() end
...
function script_switch(kind,name) end
...
function luaExitIfCall(flag) end
...
---@return table @转换后的结果
function toTableType(color,posandcolor,x,y) end
...
function snapshot(picname,x1,y1,x2,y2,quality) end
...
function findImageInRegionFuzzy(picpath,degree,x1,y1,x2,y2,alpha,kind,value) end

--- 
---  获取触动账号 ID（仅支持 Android）
-- - 引擎版本：Android v2.3.6 以上版本
---@return string @未登录账号使用此函数返回为空，登录账号后会返回触动账号 ID
function getTSAccountID() end
...
---@return number @找到的图片的左上角顶点坐标，如未找到则返回 -1,-1
function findImage( picpath,x1,y1,x2,y2,deviation) end
...
function saveToAudio(text,tab) end
...
---@return number @字库标识
function addTSOcrDict(dict) end
...
---@return string @识别结果
---@return table @坐标
function ocrText(x1,y1,x2,y2,language,whitelist,flag) end
...
function findTextUntil(text,x1,y1,x2,y2,language,list,ms,timeout) end
...
function showFloatButton(flag) end
...
function getFloatButtonPos() end
...
function tsOcrText(index,x1,y1,x2,y2,c0c1,alpha) end
...
function setFloatButtonPos(dir,per) end
...
function tsFindText(index,str,x1,y1,x2,y2,c0c1,alpha) end
...
function getRndStr(str1) end

--- 
--- 获取随机数
-- - 函数功能：获取真随机数，随机数值
---@return number @随机范围的整数，每次获取数字不同
function getRndNum() end
...
---@return number @字库标识
function addDmOcrDict(dict) end
...
---@return table @分割形成的表
function string.split(str,symbol) end
...
---@param pos number @起始分割位置，缺省默认值 1
---@return table @分割后形成的表
function string.Split(str,symbol,pos) end
...
---@param alpha number @精确度，范围 0 - 100
---@return number @找到的字符串坐标（字符串左上角点），如未找到则返回 -1,-1
function dmFindText(index,str,x1,y1,x2,y2,c0c1,alpha) end
...
---@param alpha number @精确度，范围 0 - 100
---@return string @返回的识别结果
function dmOcrText(index,x1,y1,x2,y2,c0c1,alpha) end

--- 
--- 日志
-- - 函数功能：记录脚本日志，便于了解挂机详情
---@param content string @日志内容，number 类型仅支持 TSLib v1.2.4 及其以上版本，boolean 类型仅支持 TSLib v1.3.3 及其以上版本，其他使用方法详见函数用例介绍
---@param name? string/number @日志名称，不写默认为 “脚本日志”，number 类型仅支持 TSLib v1.2.4 及其以上版本
function log(content,name) end
...
---@param content string @代码所在的行数/日志内容，在该参数中加入"[LINE]"自动替换为代码所在的行数，仅支持引擎版本 Android v3.1.8.1、iOS v3.1.3 及其以上版本，仅支持 lua 源码，tsp 脚本显示为 [0]
function nLog(content) end
...
function dialog(text,timeout) end
...
---@return table @编码后的数据
function ts.json.decode(str) end
...
function dialogInput(title,format,btn) end
...
function ts.json.encode(str1) end
...
function dialogRet(text,cbtn,btn1,btn2,time) end
...
---@param content string @需要显示的内容，number 及 boolean 类型仅支持引擎版本Android v3.1.7.1、iOS v3.1.1 及其以上版本，仅支持 v1.3.3 及其以上版本 TSLib，其他用法详见函数用例
function mLog(content) end
...
function  ts.json.encode(tb) end
...
function initLog(name,flag) end

--- 
--- wLog 输出日志
-- - 函数功能：写出或发送日志
---@param name string @日志文件名或服务器地址与端口不带后缀，自动添加 .log
---@param content string @代码所在的行数/日志内容，在该参数中加入"[LINE]"自动替换为代码所在的行数，仅支持引擎版本 Android v3.1.8.1、iOS v3.1.3 及其以上版本，仅支持 lua 源码，tsp 脚本显示为 [0]；在该参数中加入"[DATE]"自动替换为当前可读时间，number 及 boolean 类型引擎版本仅支持Android v3.1.7.1、iOS v3.1.1 及其以上版本
function wLog(name,content) end
...
function string.tohex(str1) end
...
--- sysLog 系统日志
-- - 函数功能：输出系统日志
---@param content string @需要显示的日志内容，number 及 boolean 类型引擎版本仅支持Android v3.1.7.1、iOS v3.1.1 及其以上版本
function sysLog(content) end
...
function string.fromhex(str1) end
...
function fwShowWnd(wid, x1,y1,x2,y2,kind) end
...
function image.newTextImage(w,h,str,tab) end
...
function string.fromgbk(str1) end
...
function  tap(x,y,ms,pic,index) end

--- 获取无障碍权限
function isAccessibilityOn() end
...
function moveTo(x1,y1,x2,y2,step,ms,index,stop) end
...
function image.image.loadFile(path) end
...
function randomTap(x,y,r,pic,index,ms) end
...
function moveZoomOut(x1,y1,x2,y2,step) end
...
-- - 引擎版本：iOS 专业版，Android v3.0.1 以上
---@param newImage userdata @需要操作的图片对象
---@return string/nil @图片对象的 JPEG 格式数据，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image.jpegData(newImage) end
...
---@return nil/string @成功返回空，失败返回失败原因
function image.pngData(newImage) end
...
---@return userdata/nil @截取的图片对象，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image:crop(img1,x1, y1, x2, y2) end
...
-- - 函数功能：从数据创建图片对象
-- - 引擎版本：iOS 专业版，Android v3.0.1 以上
---@param Maltmage string @图片对象的 JPEG 格式数据
---@return userdata @对象
function image.loadData(Maltmage) end
...
---@param x number @屏幕横坐标
---@param y number @屏幕纵坐标
function touchMove(index,x ,y) end
...
-- - 函数功能：从图片对象创建拷贝图片对象
-- - 引擎版本：iOS 专业版，Android v3.0.1 以上
---@param img1 userdata @需要复制的图片对象
---@return userdata/nil @复制后的图片对象，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image.copy(img1) end
...
-- - 函数功能：判断对象是否为图片对象
-- - 引擎版本：iOS 专业版，Android v3.0.1 以上
---@param img userdata @对象
---@return boolean @true - 是图片对象；false - 不是图片对象
function image.is(img) end
...
function setScreenScale(flag,width,height) end
...
function xyRotate(x1,y1,dir) end
...
function catchTouchPoint(touchCount) end
...
-- - 函数功能：从图片创建拉伸后的图片对象
-- - 引擎版本：iOS 专业版，Android v3.0.1 以上
---@param img userdata @需要操作的图片对象
---@param width number @横向拉伸后图片的宽度
---@param heigth number @纵向拉伸后图片的高度
---@return userdata/nil @拉伸后的图片对象，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image.cvResize(img,width,heigth) end
...
---@param index? table/number @手指序号，仅支持函数库 v1.3.0 及其以上版本默认不写随机手指序号，推荐使用 1 - 9
---@param ms? number @弹起时间，不写默认为 50 毫秒，仅支持函数库 v1.3.5 及其以上版本
function randomsTap(x,y,r,pic,index,ms) end
...
-- - 函数功能：销毁一个图片对象
-- - 引擎版本：iOS 专业版，Android v3.0.1 以上
---@param img1 userdata @需要销毁的图片对象
---@return nil @销毁后的对象
function image.destroy(img1) end
...
function imageBinaryzation(picpath,value) end
...
---@return userdata/nil @二值化后的图片对象
---@return nil/string @成功返回空，失败返回失败原因
function image.binaryzation(newImage, tab) end
...
function moveZoomIn(x1,y1,x2,y2,step) end
...
function image.cvBinaryzation(img,value) end
...
function moveCircleCenter(cX,cY,x,y,angle,step,direction) end
...
function ts.img.binaryzationImg(imgPath,value) end
...
---@param value number @偏色值
function ts.img.binaryzation(x1,y1,x2,y2,value) end
...
function getAngle(x1,y1,x2,y2) end
...
function ts.imgFindColor(path,color,posandcolors,x1, y1, x2, y2) end
...
---@param imgPath string @图片路径
---@return table @颜色表
function ts.img.loadFile(imgPath) end
...
---@param y2 number @图片右下角纵坐标
---@return table @颜色表
function ts.img.screen(x1,y1,x2,y2) end
...
function widget.setText(wid,text) end
...
---@param back table @错误回调，当执行任务时发生错误异常，则会回调这个函数并不再抛出
---@return number @协程 ID,ID 可用于结束或是等待一个任务
function thread.createSubThread(task,back) end
...
-- - 引擎版本：仅  Android v3.2.1 及其以上版本支持
---@param wid userdata @控件对象
function widget.scrollForward(wid) end
...
function thread.clearTimeout(thread_id) end
...
---@param thread_id number @协程 ID，如果在协程内，参数省略
function thread.setTimeout(timeout,thread_id) end

--- 
--- 协程阻塞等待
-- - 函数功能：阻塞等待所有协程结束，只能在主线程中使用
-- - 引擎版本：iOS v2.3.9,Android v2.3.4 以上
function thread.waitAllThreadExit() end
...
-- - 引擎版本：仅  Android v3.2.1 及其以上版本支持
---@param wid userdata @控件对象
function widget.scrollBackward(wid) end

--- 
--- 模拟按下返回键
-- - 函数功能： 点击一次返回键
-- - 引擎版本：仅 Android v3.2.2 及其以上版本支持
function keycode.back() end

--- 
--- 模拟按下 home 键
-- - 函数功能： 点击一次 home 键
-- - 引擎版本：仅 Android v3.2.2 及其以上版本支持
function keycode.home() end
...
function VPNVersions() end

--- 
--- 获取设备存储空间
-- - 函数功能：获取设备存储空间
---@return string @剩余容量G/ 总容量G
function ts.system.space() end
...
function closeApp(bid,kind) end

--- 
--- 拉出通知栏
-- - 函数功能： 下滑拉出通知栏
-- - 引擎版本：仅 Android v3.2.2 及其以上版本支持
function keycode.notification() end
...
-- - 引擎版本：Android v2.1.1 以上
---@return string @设备型号
function getDeviceModel() end

--- 
--- 获取前台应用
-- - 函数功能：获取前台应用的 Bundle ID 或包名，安卓和 iOS 使用有区别
---@return string @返回前台应用程序的包名，若无应用处于前台，则返回空字符串
---@return string @返回前台应用程序的包名及类名(andorid)
function frontAppBid() end
...
function getIMEI() end

--- 
--- 获取 IMEI
-- - 函数功能：获取设备 IMEI
---@return string @设备 IMEI
function ts.system.imei() end
...
function appBundlePath(bid) end
...
function getDeviceName() end

--- 
--- 获取网络类型 
-- - 函数功能：用于判断设备网络类型 
---@return string @wwan（包括 2G、3G、4G）/WiFi/NO NETWORK
function ts.getNetType() end
...
function setDeviceName(name) end
...
---@return number @0 - 安装失败；1 - 安装成功
function install(path) end

--- 
--- 获取本地网络地址
-- - 函数功能：获取本地网络地址
---@return table @本地网络地址，包含本地内部环回地址、蜂窝地址、本地 WiFi 地址
function ts.system.localwifiaddr() end

--- 
--- 获取 SD 卡路径
-- - 函数功能：获取设备 SD 卡路径
---@return string @返回设备 SD 卡路径，无 SD 卡时返回 nil
function getSDCardPath() end
...
function isInstalledApk(bid) end

--- 
--- 获取蓝牙 MAC 地址
-- - 函数功能：获取设备蓝牙 MAC 地址
---@return string @蓝牙 MAC 地址
function ts.system.btmac() end

--- 
--- 获取设备 WiFi MAC 地址
-- - 函数功能：获取设备 WiFi MAC 地址
---@return string @设备 WiFi MAC 地址
function ts.system.wifimac() end
...
function closeAllApp(bid) end

--- 
--- 获取屏幕的 render 值
-- - 函数功能：获取屏幕的 render 值
---@return number @设备的 render 值
function getRender() end

--- 
---  获取指定应用的信息（仅支持 Android）
-- - 引擎版本：Android v2.3.6.1 以上
-- - 函数功能：获取指定应用的信息
---@param bid string @包名，可以使用frontAppBid函数获取
---@return table @应用信息
function getAppInfo(bid) end
...
function getStatusBarHeight() end
...
---@param oldpath string @要拷贝的文件路径
---@param newpath string @新的文件路径
---@return boolean @true - 复制成功，false - 复制失败
function hlfs.copyFile(oldpath,newpath) end
...
---@param newpath string @新的文件路径
---@return boolean @true - 复制成功，false - 复制失败
function hlfs.copyDir(oldpath,newpath) end
...
function hlfs.makeDir(path) end
...
function ts.hlfs.getFileList(path,bool) end
...
function pressHomeKey(state) end
...
function delFile(file) end

--- 
--- 双击 Home 键
-- - 函数功能：模拟用户双击 Home 键的操作
function doublePressHomeKey() end
...
function hlfs.removeEntryDir(path) end
...
function setRotationLockEnable(flag) end
...
---@return string @返回头部
---@return string @返回内容
function http.get(url,tb) end

--- 
--- 打开最近任务
-- - 函数功能： 下滑显示最近任务
-- - 引擎版本：仅 Android v3.2.2 及其以上版本支持
function keycode.recent() end

--- 
--- 显示快速设置
-- - 函数功能： 下滑显示快速设置
-- - 引擎版本：仅 Android v3.2.2 及其以上版本支持
function keycode.quickSetting() end
...
function httpPost(url,data,options) end

--- 
--- 设备分屏
-- - 函数功能： 分屏显示画面
-- - 引擎版本：仅 Android v3.2.2 及其以上版本支持
function keycode.splitScreen() end
...
---@return string @返回头部
---@return string @返回内容
function ts.httpPost(url,header_send,body_send,encode) end
...
---@param header_send table @请求头部内容，本参数必须与请求体一起填写
---@param body_send table @请求内容，本参数必须与头部一起填写
---@param tab table @table 参数(andorid)
---@return number @返回状态码
---@return string @头部返回内容
---@return string @返回内容
function ts.httpsPost(url,header_send, body_send,tab) end
...
function inputText(str,kind) end

--- 
--- 弹出电源键菜单
-- - 函数功能： 显示电源键菜单
-- - 引擎版本：仅 Android v3.2.2 及其以上版本支持
function keycode.power() end
...
---@return boolean @成功返回 true，失败返回 false
function ts.ftp.connect(ftppath,username,password,kind) end
...
function  switchTSInputMethod(flag) end
...
function ts.setHttpProxy(IP,port) end
...
function ts.ftp.upload(localfilepath,severfilepath,kind) end
...
function ts.ftp.download(localfilepath,severfilepath,kind) end
...
function  inputKey(str,time_min,time_max) end
...
---@return boolean @true - 删除成功，false - 删除失败
function ts.ftp.delete(fileName) end
...
function getColorRGB(x,y) end

--- 
--- 重命名文件
-- - 函数功能：重命名服务器上指定目录的文件
---@param oldfilepath string @待重命名文件名
---@param newfilepath string @新文件名
---@return boolean @true - 重命名成功，false - 重命名失败
function ts.ftp.rename(oldfilepath,newfilepath) end
...
function ts.ftp.list(filepath) end
...
function multiColor(array,dim,flag) end
...
---@return boolean @true - 创建成功，false - 创建失败
function ts.ftp.mkdir(filePath) end
...
---@return boolean @true - 全部坐标和颜色一致，false - 坐标和颜色值不符
function multiColTap(array,dim,pic,flag) end
...
---@return boolean @true - 删除成功，false - 删除失败
function ts.ftp.rmdir(filepath) end
...
---@return boolean @true - 存在；false - 不存在
function muColors(color_table1,color_table2,options) end
...
---@param dim? number @精度，不写默认为 90
---@return boolean @true - 存在，false- 不存在
function isColor(x,y,c,dim) end
...
---@param username string @用户名，服务器登录用户名
---@param password string @密码，服务器登录密码
function ftp.init(url,username,password) end
...
---@return boolean @true - 存在；false - 不存在
function findColorUntil(x,y,color,bool,dim,ms,timeout) end
...
function findColorInRegionFuzzy(color,degree,x1,y1,x2,y2) end
...
function ftp.upload(localfilepath,severfilepath) end

--- 
--- 设置 FTP 超时时间
-- - 函数功能：设置 FTP 超时时间，超时后执行下一步防止阻塞
---@param sec number @超时时间，单位为秒
function ftp.setTimeout(sec) end
...
function ftp.download(localfilepath,severfilepath) end
...
function ftp.rename(oldfilepath,filepath) end
...
function ftp.delete(filepath) end
...
---@param timeout? number @超时时间，不写默认为 60 秒
---@return boolean @true - 存在；false - 不存在
function findColorsUntil (color,posandcolors,degree,x1,y1,x2,y2,tab,ms,timeout) end
...
function ftp.list(filepath) end
...
---@return string @删除失败返回失败原因
function ftp.rmkdir(filepath) end
...
function ftp.mkdir(filePath) end

--- 
--- getColor 获取图片对象某点颜色
-- - 函数功能：获取图片对象指定坐标点的颜色并返回颜色值和不透明度
---@param newImage userdata @图片对象
---@param x number @将获取颜色值的屏幕横坐标
---@param y number @将获取颜色值的屏幕纵坐标
---@return number @颜色值
---@return number @坐标的不透明度，取值范围 0 ~ 255
function image.getColor(newImage,x,y) end
...
---@return userdata/nil @图片对象，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image.setColor(newImage,x,y,rgb) end
...
function image.isColors(img,array,dim) end
...
function image.findColor(img,tab,degree, x1, y1, x2, y2) end
...
---@return string @下载状态
function ts.tsDownload(path,url,bool) end
...
function connect(name,username,password,IP,port) end
...
---@return number @绿
---@return number @蓝
function intToRgb(color) end

--- 
--- replaceColor 颜色替换
-- - 函数功能：将图片上的指定颜色替换为其他颜色并返回结果
---@param img1 userdata @需要操作的图片对象
---@param rgb1 number @需要替换的颜色值
---@param rgb2 number @替换后的颜色值
---@param degree? number @匹配度，不写默认为 0
---@return userdata/nil @颜色替换后的图片对象，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image.replaceColor(img1,rgb1,rgb2,degree) end
...
function unzip(zipPath,filePath) end
...
function zip(zipPath,filePath) end
...
function execute(sentence) end

--- 
--- 二维码图片生成
-- - 函数功能：将文本编译成二维码图片
---@param path string @保存路径
---@param str string @二维码信息内容
---@param size number @二维码图片尺寸
---@return string @返回状态，true- 生成二维码成功，false - 生成二维码失败
function ts.qrEncode(path,str,size) end
...
function ts.qrDecode(path) end

--- 
--- qrDecode 解码一个二维码图片
-- - 函数功能：将二维码图片解析为字符串
---@param img userdata @需要解析的图片对象
---@return string/nil @解析二维码得到的字符串，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image.qrDecode(img) end
...
function saveVideoToAlbum(path) end

--- 
--- 清空剪贴板
-- - 函数功能：清除设备剪贴板信息
function clearPasteboard() end
...
function snapAndSave(x1,y1,x2,y2) end

--- 
--- saveToAlbum 保存图片对象到相册
-- - 函数分类：tsimg 相关函数
-- - 函数功能：保存图片对象到相册
---@param img userdata @需要保存的图片对象
---@return boolean @true - 保存成功；false - 保存失败
---@return nil/string @成功返回空，失败返回失败原因
function image.saveToAlbum(img) end

--- 
--- 清空相册
-- - 函数功能：删除手机相册所有照片
function clearAllPhotos() end

--- 
--- 清除钥匙串
-- - 函数功能：清除指定应用储存在系统钥匙串中的信息
---@param bid string @需要清除的应用程序的 Bundle ID
function clearKeyChain( bid) end
...
function clearAllKeyChains(tb) end
...
function clearIDFAV(info) end

--- 
--- 清除浏览器 Cookies
-- - 函数功能：清除浏览器 Cookies 信息
function clearCookies() end
...
-- - 引擎版本：iOS 专业版，Android v3.0.1 以上
---@param img userdata @需要转换的图片对象
---@param path string @需要保存的文件路径，只支持绝对路径
---@param quality number @此参数控制图片质量，参数范围：0.1 - 1，不写、小于 0 或者大于 1 默认为 1
---@return boolean @true - 保存成功；false - 保存失败
---@return nil/string/ @成功返回空，失败返回失败原因
function image.saveToJpegFile(img,path,quality) end

--- 
--- 清空通讯录
-- - 函数功能：删除通讯录所有联系人
-- - 引擎版本：iOS v2.3.6,Android v2.0.5 以上
function removeAllContactsFromAB() end
...
---@return boolean @true - 保存成功；false - 保存失败
---@return nil/string @成功返回空，失败返回失败原因
function image.saveToPngFile(img,path) end
...
-- - 函数功能：图片对象顺时针旋转 90 度
-- - 引擎版本：iOS 专业版，Android v3.0.1 以上
---@param newImage1 userdata @需要进行操作的对象
---@return userdata/nil @旋转 90 度后的图片对象，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image.setRotation90(newImage1) end
...
-- - 引擎版本：iOS 专业版，Android v3.0.1 以上
---@param newImage1 userdata @需要进行操作的对象
---@return userdata/nil @旋转 180 度后的图片对象，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image.setRotation180(newImage1) end
...
function ts.imgSize(path1,path2,w,h) end
...
function image.operMerge(tab,path,dir,quality) end
...
function imageOperMerge(pictb, path,kind,quality) end
...
-- - 引擎版本：iOS 专业版，Android v3.0.1 以上
---@param newImage1 userdata @需要进行操作的对象
---@return userdata/nil @旋转 270 度后的图片对象，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image.setRotation270(newImage1) end
...
-- - 引擎版本：iOS 专业版，Android v3.0.1 以上
---@param img1 userdata @需要贴图的图片对象
---@param img2 userdata @需要贴的图片对象
---@param tab table @贴图设置参数
---@return userdata/nil @贴图后的图片对象，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image.drawImage(img1,img2,tab) end
...
---@param w number @图片宽度
---@param h number @图片高度
---@return userdata/nil @创建的图片对象，失败显示空
---@return nil/string @成功返回空，失败返回失败原因
function image.new(w,h) end
...
function fwShowImageView(wid,vid,picpath,x1,y1,x2,y2,alpha,radius) end
...
--- 
--- 获取 VPN 列表，返回一个 table
---@return string @vpn 名称列表
function getVPNList() end

--- 
--- BASE64 编码 
-- - 函数功能：字符串 BASE64 编码（模式为 ECB）
function string.base64_encode(str) end
...
function fwGetPressedButton() end
...
---@param key string @密匙
function string.aes128_decrypt(tmp,key) end
...
function fwCloseView(wid, vid) end
...
-- - 引擎版本：iOS v2.3.5,Android v2.0.3 以上
---@param wid string @需要关闭的父窗口载体的 ID
function fwCloseWnd(wid) end

--- 
--- 字符串 BASE64 解码（模式为 ECB）
function string.base64_decode() end
...
function closeFw() end

--- 
--- UI 创建初始化
-- - v3.2.0
-- - 3.1.5
function UINew(pagination,titles,okname,cancelname,config,orient,timer,width,height,bgcolor,btnbkcolor,bg,pagenumtype,selpage,titlesize, titlealign) end
...
function showTextView(text,id,x1,y1,x2,y2,align,color,bgcolor,size,kind,alpha,fwType,radius) end
...
function utf8.codepoint (str,i,j) end
...
function utf8.offset (str,n,i) end
...
---@return table @复制后的表
function deepCopyTable(tmp) end
...
function string.sha1(str1) end

--- 
--- 单选组合
-- - 函数功能：单项选择返回
function  UIRadio(num,id,list,sel,width,nowrap,images,scale,countperline) end
...
function string.md5(str1) end

--- 
--- 多行文本框
-- - TSLib 版本：仅支持函数库 v1.2.4 及其以上版本
-- - 函数功能：一般来用显示多行说明文字或者标题
function  UITextArea(num,id,prompt,text,size,align,color,kbtype,width,nowrap,height) end
...
function ocrInfo(platform,username,password) end
...
function ocrScreen(x1,y1,x2,y2,kind,timeout,scale) end
...
function ocrBalance() end

--- 
--- 下拉框
-- - 函数功能：单项选择返回
function UICombo(num,id,list,sel,width,nowrap,prompt) end

--- 
--- 多选组合
-- - 函数功能：多项选择返回
function UICheck(num,id,list,sel,width,nowrap,images,scale,countperline) end
...
function ocrReportError(id) end
...
---@return string|boolean @正确返回打码结果，错误返回 nil
---@return string @正常返回 id，错误返回错误信息
function ocrImage(path,kind,timeout,scale) end

--- 
--- 关联框
-- - 函数功能：可作为 UIComboRlt 的子对象，通过标志项 dataSource 与 UIComboRlt
function UIComboRlts(num,id,dataSource,sel,width,nowrap,prompt) end

--- 
--- 联动框
-- - 函数功能：单项选择返回，该函数与联动关联框配合使用
function UIComboRlt(num,id,list,data,source,sel,width,nowrap,prompt) end

--- 
--- 显示网页
-- - 引擎版本：仅支持 Android v3.1.3,iOS v3.0.6 及其以上版本
-- - 其他版本要求：仅支持 TSLib  v1.2.9 及其以上版本
function UIWeb(num,url,width,height) end
...
-- - 引擎版本：仅支持 Android v3.1.3,iOS v3.0.6 及其以上版本
-- - 其他版本要求：仅支持 TSLib v1.2.9 及其以上版本
function UILine(num,align,width,height,color) end
...
---@param flag boolean @true - 打开保持屏幕，false - 关闭保持屏幕
function keepScreen(flag) end
...
function neosUI(tab,Explist) end

--- 
--- getOSVer 获取系统版本号
-- - 函数功能：获取系统版本号
---@return string @返回系统版本号如：8.3
function getOSVer() end
...
function checkTable(UITable) end
...
function UISwitch(num,id,state,size,align,width,nowrap) end

--- 
--- 获取系统版本号
-- - 函数功能：获取设备系统版本号
---@return string @设备系统版本号
function ts.system.osversion() end

--- 
--- 获取详细型号
-- - 函数功能：获取设备详细型号
---@return string @设备详细型号
function ts.system.devicetype() end
...
function getDeviceType() end

--- 
--- 显示 webview
-- - 函数功能：显示一个 webview
-- - 引擎版本：支持 iOS v2.3.9,Android v2.3.4 以上
function myweb1.show() end
...
function event.register(event,func) end

--- 
--- 关闭 webview
-- - 函数功能：关闭一个 webview
-- - 引擎版本：iOS v2.3.9,Android v2.3.4 以上
function myweb1.close() end

--- 
--- 获取设备序列号
-- - 函数功能：获取设备序列号
---@return string @设备序列号
function ts.system.serialnumber() end
...
---@return boolean @true - 删除成功，false - 删除失败
function hlfs.removeDir(path) end
...
function writeFile(file,tab,mode,wrap) end
...
function readFileString(file) end
...
---@return boolean @true - 写入成功；false - 写入失败
function writeFileString(file,str,mode,wrap) end
...
function readUIConfigFile(path) end
...
function plist.read(plfilename) end
...
function  writeUIConfigFile(path,textList) end
...
function plist.write(plfilename, tmp2) end
...
---@return string @文件格式
function hlfs.getFileEncoding(filePath) end
...
function hlfs.codeConvert(srcPath,dstPath,mode) end
...
function checkTSLibrary(name) end
...
function compareVersion(v1,v2) end
...
---@return string/boolean @成功返回网页内容，失败返回 false
function httpGet(url,timeOut) end
...
---@return string @返回头部
---@return string @返回内容
function ts.httpGet(url, header_send, body_send) end
...
---@return string @头部返回内容
---@return string @返回内容
function ts.httpsGet(url, header_send, body_send) end
